...
class ResetPasswordNotifier:
    ...
    def _build_link(self, base_url: URL, token: str) -> str:
        return f"{base_url}{self.reset_link_path}?token={token}"
    ...
    async def send_password_reset_email(
        ...
        if self.redis_client is None:
            raise RuntimeError("Redis client is required for reset password tokens.")
...
def get_reset_password_notifier(
    ...
    return ResetPasswordNotifier(email_service=email_service, redis_client=redis_client)
