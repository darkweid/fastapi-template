...
- CD (`.github/workflows/prod_deploy.yml`, `stage_deploy.yml`) normally triggers automatically once the matching CI succeeds — `CI (prod)` on `main`, `CI (stage)` on `stage`. Each also accepts a manual `workflow_dispatch` run (Actions tab → *CD (prod)* / *CD (stage)* → *Run workflow*, or `gh workflow run prod_deploy.yml`) — the same `*_DEPLOY_ENABLED` gate and the `deploy-<environment>` concurrency group apply, so a manual run still queues behind an in-flight automatic one for that environment, and never behind the other environment's. Its optional `image_tag` input deploys a specific already-built image tag; left blank, it computes `sha-<12>` from the dispatched commit. Because `workflow_run` always runs the copy of the workflow file on the default branch, edits to either CD file only take effect once merged into `main`.
...
- The box needs `docker login ghcr.io` credentials for a private package (`GHCR_USER` / `GHCR_PULL_TOKEN` in that environment's secrets). Postgres stays a box-local build — CD ships application code, never the database image.
