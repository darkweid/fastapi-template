...
- Probes: `/live/` has no dependencies and is what the `app` container healthcheck polls, so a Postgres or Redis outage never restarts a healthy container. `/ready/` returns 503 while Postgres is unreachable — the gate to put in front of a load balancer. `/health/` is the detailed report for monitoring and stays 200 with `"status": "degraded"` when only Redis is down. None of them reports to Sentry: they run on a timer, and a single outage would file one event per poll.
- `worker` and `scheduler` have no HTTP surface, so their healthcheck pings the broker Redis from inside the container — a wrong password or an unreachable broker shows up as `unhealthy` in `docker ps` instead of a silently idle container.
