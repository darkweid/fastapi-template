...
## Deployment Notes
- `infra/deploy/deploy.sh` is the single deploy path, run on the box: it validates `.env`, tests the nginx configuration it ships in a throwaway container (`nginx -t` with the new mounts, skipped on a first deploy with no app container to resolve), brings up Postgres and Redis, applies migrations, then rolls `app`, `worker`, `scheduler` and restarts nginx. Migrations run before any new code serves traffic, and a failed one aborts the deploy with the previous containers still up; so does a failed nginx test.
