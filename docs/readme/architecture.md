...
- **Paging:** `page` runs from 1 to `MAX_PAGE` (100 000) and `size` from 1 to 100; anything outside answers 422. The ceiling on `page` keeps `(page - 1) * size` inside the bigint `OFFSET` is bound as, which would otherwise overflow in the driver and answer 500.
...
- **Period bounds:** `date_from` and `date_to` take a datetime or a bare date (`2026-09-24`). A bare `date_from` is the start of that day; a bare `date_to` is the whole of that day, bound as `< next midnight`, so `date_from=2026-09-24&date_to=2026-09-24` selects one day. A datetime `date_to` stays inclusive. Bare dates and naive datetimes are read in `ListQuery.local_timezone`, UTC unless a project whose users count days in a local calendar sets it; an offset-aware value keeps its own offset, so a client may mix the two. Both bounds are converted to UTC instants before they are compared or bound, matching the `DateTime(timezone=True)` columns every timestamp in this project uses. A bound outside the calendar (`date_to=9999-12-31`, whose next midnight does not exist) is a `FilteringError`, not a 500.
