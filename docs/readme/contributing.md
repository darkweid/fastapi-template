...
- The `changes` gate classifies the diff through `.github/actions/docs-only-change` and switches off every job but `lint` and `gitleaks` when each changed path is documentation (`*.md`, `docs/`, `LICENSE`). The rule lives in `scripts/docs_only_change.py`, is deny-by-default, and is widened there together with a case in `tests/unit/scripts/test_docs_only_change.py` — a wrong `true` reads as a green PR with nothing run.
