...
- Both gates call the local action `.github/actions/docs-only-change`, resolved from the checked-out commit. A `stage` branch created before that directory existed fails the CD step until it merges `main`.
...
- A tree is checked once. When every check passes, `record-pass` uploads an artifact named `ci-passed-<tree id>`; on a push, the `changes` gate looks that name up for the pushed commit's tree and, if a run of this repository's own branches recorded it, skips every check except `gitleaks`, which scans history rather than the tree, and lets `build-and-push` run. A squash merge of a pull request tested against the current base has exactly that tree, and so does a fast-forward of another branch to an already-checked commit. If the base moved after the pull request's last run, the tree differs and the push runs everything. Markers live 30 days; the lookup needs `actions: read`, granted by both callers.
...
- Runs the same docs-only classification on the commit it is about to deploy and skips the SSH steps when it answers `true`: CI built no image for such a commit, so there is no `sha-` tag to pull. A `workflow_dispatch` is never gated.
