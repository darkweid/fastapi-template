...
- A tree is checked once. When every check passes, `record-pass` uploads an artifact named `ci-passed-<tree id>`; on a push, the `changes` gate looks that name up for the pushed commit's tree and, if a run of this repository's own branches recorded it, skips every check and lets `build-and-push` run alone. A squash merge of a pull request tested against the current base has exactly that tree, and so does a fast-forward of another branch to an already-checked commit. If the base moved after the pull request's last run, the tree differs and the push runs everything. Markers live 30 days; the lookup needs `actions: read`, granted by both callers.
